...
namespace {
    ...
    void planLaneAndSpeed(
        ...
        for(size_t i = 0; i < sensor_fusion.size(); ++i) {
            int car_i_lane = getLane(sensor_fusion[i][6]);
            double car_i_future_s = vehicle.getFuturePosition(sensor_fusion[i]);
...
namespace path_planning {
    ...
    void generateTrajectory(
        ...
        // step 2: setup target points using 30m apart in Frenet (start from last planned point) 
        // then convert to Cartesian
        std::vector<double> x_vals;
        std::vector<double> y_vals;
        vehicle.getSplinePoints(x_vals, y_vals, highway_map, next_lane, 30);
